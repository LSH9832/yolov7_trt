    ...
    for (size_t c = 0; int(c) < channels; c++)
        for (size_t  h = 0; int(h) < img_h; h++)
            for (size_t w = 0; int(w) < img_w; w++)
                blob[c * img_w * img_h + h * img_w + w] = (float)img.at<cv::Vec3b>(h, w)[c] / 255;
    return blob;
}

static void decode_outputs(float* prob, std::vector<Object>& objects, float scale, const int img_w, const int img_h, float conf_thres, float nms_thres, int img_size, int num_class, int num_array) {
...
//    cout<<bitrate<<endl;
    ...
    int output_num_array = output_size / (5 + num_class);
    ...
    if(sourcetype==0){
        ...
        decode_outputs(prob, objects, scale, img_w, img_h, conf_thres, nms, img_size, num_class, output_num_array);
...
//            cv::Mat out;
            if (objects.size() > 0) {
                ...
                for (size_t i = 0; i < objects.size(); i++)
                {
                    ...
                    baseLine = 0;
                    label_size = cv::getTextSize(text, cv::LINE_AA, 0.66, 2, &baseLine);

                    txt_bk_color = color; // * 0.7;

                    x = obj.rect.x;
                    y = (y > image.rows)?image.rows:obj.rect.y + 1;
            ...
            delete blob;
            auto end = std::chrono::system_clock::now();
            std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end - start2).count() << "ms" << std::endl;
        }
        if (push) pclose(fp);
